  ...
  <div class="wrapper">
    ...
    <el-row :gutter="10">
      <el-col :span="14">
        <div>
          ...
          >
            ...
            <!-- <el-form-item label="充值类型" prop="idCard" v-if="isShowForm">
              <div
                class="radio-btn"
                ...
                :key="i.key"
              >
                <el-radio v-model="form.id" name="type" :label="i.id">{{
                  i.channelType
                }}</el-radio>
              </div>
            </el-form-item>
            <div v-if="isShowForm && form.type === 0">
              ...
              <el-row>
                <el-button type="text" @click="toLook"
                  >查看支付宝限额说明</el-button
                >
              </el-row>
            </div>
            <div v-else-if="isShowForm && form.type === 1">
              <el-form-item label="收款银行" prop="name">
                <el-input
                  disabled
                  type="text"
                  v-model="info.channelDesc"
                  placeholder="收款银行"
                >
                </el-input>
              </el-form-item>
            ...
            <el-form-item v-if="isShowForm" label="打款凭证" prop="vouImage">
              <el-row>
                <el-col :span="10">
                  <el-upload
          ...
          <div
            v-if="form.type === 0 && info.channelImg"
            class="pay-box text-center"
          >
            <img class="eq-code" :src="info.channelImg" alt="" />
          </div>

          <div
            v-if="isShowForm && form.type != 2"
            slot="footer"
            class="text-center dialog-footer"
          >
            <!-- <el-button
              type="primary"
              v-if="payShow"
      ...
      <el-col :span="10">
        <div v-if="form.type === 0">
          <div class="tips-group">
            ...
            <p class="tip-text">
              ...
              >由于支付收款上限限制，每次收款公户可能不一样，<span class="red"
                >请每次充值前获取最新的二维码完成支付</span
              >
            </p>
            <p class="tip-text">
              <i class="iconfont icon-jingpaibuzhou3"></i
              >如果您的网络环境不稳定，或由于设备、环境、偏好、行为、关系、账户、身份等维度问题，<span
                class="red"
                >可能导致支付宝风控系统提示风险，请您在安全的环境下发起支付</span
              >
            </p>
            <p class="tip-text">
              <i class="iconfont icon-jingpaibuzhou2"></i
              >为确保入金及时到账，<span class="red"
          ...
          <div v-if="false" class="tips-group">
            ...
            <p class="tip-text">
              ...
              >将二维码保存到本地，打开支付宝扫描二维码，转账到平台指定对公账户
            </p>
            <p class="tip-text">
              <i class="iconfont icon-buzhou2"></i
              >点击“复制”，复制支付宝账号，打开支付宝点击转账，转账到平台指定对公账户
            </p>
          </div>
        </div>
        ...
        <div v-show="form.type === 1" class="pay-box text-center">
          <!-- <img class="img" :src="info.channelImg" alt=""> -->
        </div>
      </el-col>
    </el-row>
    <!-- <div class="attention">
      <p>
        注意:
        充值默认充值在融资账户中,如需充值指数账户可从融资账户转入至指数账户 。
      </p>
    </div> -->
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="payh5status = false"
          >充值完成</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      admin: "",
      rule: {
        number: [{ required: true, message: "请输入充值金额", trigger: "blur" }]
      },
      info: {
        name: ""
      },
  ...
  methods: {
    handleAvatarSuccess(res, file) {
      this.form.vouImage = res.data.url;
    },
    ...
    inputChange (v) {
      this.debounce(this.getPayInfo, 2000)
    },

    async getPayInfo() {
      return false;
      if (Number(this.form.amt) >= 100) {
        this.isShowForm = true;
      }
      // 获取支付渠道
      let data = await api.getPayInfo({ payAmt: this.form.amt });
      if (data.status === 0) {

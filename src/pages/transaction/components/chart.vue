    ...
    <div class="panel-box">
      <div class="panel-title clearfix">
        ...
        <ul class="chart-title pull-right">
          ...
          <li
            @click="changeType(0)"
            :class="type == '0' ? 'on li-type' : 'li-type'"
          >
            <a class="chart-type" href="javascript:;">分时</a>
          </li>
          <li
            @click="changeType(5)"
            :class="type == '5' ? 'on li-type' : 'li-type'"
          >
            <a class="chart-type" href="javascript:;">5分钟</a>
          </li>
          <li
            @click="changeType(15)"
            :class="type == '15' ? 'on li-type' : 'li-type'"
          >
            <a class="chart-type" href="javascript:;">15分钟</a>
          </li>
          <li
            @click="changeType(30)"
            :class="type == '30' ? 'on li-type' : 'li-type'"
          >
            <a class="chart-type" href="javascript:;">30分钟</a>
          </li>
          <li
            @click="changeType(60)"
            :class="type == '60' ? 'on li-type' : 'li-type'"
          >
            <a class="chart-type" href="javascript:;">60分钟</a>
          </li>
          <li
            @click="changeType(1)"
            :class="type == '1' ? 'on li-type' : 'li-type'"
          >
            <a class="chart-type" href="javascript:;">日线</a>
          </li>
        </ul>
      </div>
      <div class="panel-body">
        <div class="chart-box" v-show="type === 0" ref="myEchart">
          <div v-if="emptyShow" class="empty text-center">
            <i class="iconfont icon-wushuju"></i>
            暂无数据
          </div>
        </div>
        <div class="chart-box" v-show="type != 0" ref="myEchart2"></div>
      </div>
    </div>
    <div class="table">
      <div class="panel"></div>
...
export default {
  ...
  methods: {
    ...
    async getDate2(val) {
      if (val === 1) {
        if (this.$route.query.code.indexOf("hf_") != -1) {
          //期货日线图
          let data = await api.getFuturesDayK({ code: this.$route.query.code });
          if (data.status === 0) {
          ...
          //指数日线图
          let data = await api.getIndexDayK({ code: this.$route.query.code });
          if (data.status === 0) {
            this.initEchartMap2(data.data);
          } else {
            Toast(data.msg);
          ...
          if (this.$route.query.code.length === 5) {
            ...
            let data = await api.getMinKEchartsGg(opts);
            if (data.status === 0) {
              this.initEchartMap2(data.data);
            } else {
              Toast(data.msg);
            ...
            //股票日线图
            let data = await api.getDayK({ code: this.$route.query.code });
            if (data.status === 0) {
              this.initEchartMap2(data.data);
            } else {
              Toast(data.msg);
    ...
    async getDate() {
      ...
      if (opts.stockCode.length === 6) {
        data = await api.getMinuteLine(opts);
      }
      if (opts.stockCode.length === 5) {
        data = await api.getMinuteLineGg({
          code: this.$route.query.code
        });
      }
    ...
    initEchartMap(data) {
      ...
      let option = {
        backgroundColor: this_.styleName === "red" ? "#fff" : "#171e2a",
        animation: true,
        // legend: { // 图例组件
            ...
            if (param && param2) {
              return [
                ...
                "涨跌幅: " +
                  ...
                  classFont +
                  '" style="font-size:16px;">' +
                  data.rates[param.dataIndex] +
                  "%</span>" +
                  "<br/>",
                "成交量(手): " + param2.value[0] / 100 + "<br/>",
                "成交额(万): " +
                  (data.amounts[param.dataIndex] / 10000).toFixed(2) +
                  "<br/>"
              ].join("");
            }
          }
        ...
        xAxis: [
          {
            ...
            axisLabel: {
              show: true,
              textStyle: {
                color: this_.styleName === "red" ? "#666" : "#ccc"
              }
            }
          },
          {
        ...
        yAxis: [
          {
            ...
            splitLine: {
              lineStyle: {
                // 使用深浅的间隔色
                color: this_.styleName === "red" ? "#eee" : "#1e242f" // 分割线
              }
            }
          },
          {
        ...
        series: [
          ...
          },
          {
            ...
            lineStyle: {
              // color:'#7cb5ec',
              color: this_.styleName === "red" ? "#7cb5ec" : "#3e5281"
              // normal: {opacity: 0.5}
            },
            itemStyle: {
              color: "#3e5281" // 线条颜色
    ...
    async addOptions() {
      let data = await api.addOption({ code: this.$route.query.code,type:this.detail.stockType==="HK"?"hk":"" });
      if (data.status === 0) {
